    ...
    public void change2(View v){
        ...
        if (pwd1.equals("")||pwd2.equals("")){
            ...
            if (pwd1.equals(pwd2)){
                ...
                //2. 网络为连接则直接返回
                if(!NetStateUtil.checkNetworkState(changepassword.this))
                    return;
                //3. 发起http请求
                OkHttpClient okHttpClient = new OkHttpClient();     //创建OkHttpClient实例

                FormEncodingBuilder builder = new FormEncodingBuilder();
                RequestBody requestBody = builder.add("name", LoginUser)
                        .add("newPassword",pwd2)
                        .build();                                          //需要传输的数据存入requestBody

                Request request = new Request.Builder()
                        .url("http://192.168.0.105:8080/cPassword")							 //需要的url
